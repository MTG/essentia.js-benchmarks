...
const modelPath = 'file://' + path.join(__dirname, '..', '..', 'models/genre_rosamerica-vggish-audioset-1/model.json');
...
Promise.all( [readFile(audioFilePath), vggishWrapper.loadModel(modelPath)] ).then((responses) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('genre_rosamerica_vggish.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing genre_rosamerica_vggish JSON Object to File.");
                return console.log(err);
            }

            console.log("genre_rosamerica_vggish JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
