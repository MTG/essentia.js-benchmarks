...
readFile(audioFilePath).then((audioBuffer) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('tensorflow_input_vggish.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing tensorflow_input_vggish JSON Object to File.");
                return console.log(err);
            }

            console.log("tensorflow_input_vggish JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
