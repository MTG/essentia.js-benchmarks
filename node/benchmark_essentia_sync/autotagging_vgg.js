...
const modelPath = 'file://' + path.join(__dirname, '..', '..', 'models/msd-vgg-1/model.json');
...
Promise.all( [readFile(audioFilePath), musicnnWrapper.loadModel(modelPath)] ).then((responses) => {
    ...
    .on('complete', function() {
        ...
        fs.writeFile('autotagging_vgg.json', json, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing autotagging_vgg JSON Object to File.");
                return console.log(err);
            }

            console.log("autotagging_vgg JSON file has been saved.");
        });
    })
    // run async
    .run({ 'async': true });       
