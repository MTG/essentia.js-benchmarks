...
fs.readFile(audioFilePath, (err, data) => {
    ...
    const rightChannelData = result.channelData[1];
    ...
    suite.add('Essentia#EBUR128', () => {
            ...
            // }
            const audioDataL = essentia.arrayToVector(leftChannelData);
            const audioDataR = essentia.arrayToVector(rightChannelData);
        ...
        .on('complete', function() {
            ...
            fs.writeFile('EBUR128.json', json, 'utf8', function (err) {
                if (err) {
                    console.log("An error occured while writing Loudness EBUR128 JSON Object to File.");
                    return console.log(err);
                }

                console.log("Loudness EBUR128 JSON file has been saved.");
            });
        })
        // run async
        .run({ 'async': true });
